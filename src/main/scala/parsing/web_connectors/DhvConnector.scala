...
case class DhvConnector(baseId: Long) extends Connector {
  ...
  def newResults: Future[(List[ParseResult], Connector)] = {
    val futureDoc = Future(browser.get("http://www.dhv.de/db3/gebrauchtmarkt/anzeigen?suchbegriff=&rubrik=0&hersteller=&muster=&preismin=&preismax=&anbietertyp=0&land=0&plz=&itemsperpage=1000&order=1"))
    futureDoc.map { doc =>
